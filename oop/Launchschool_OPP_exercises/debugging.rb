...
community_library.books.each do |element|
...
class TaskManager
  ...
  def display_high_priority_tasks
    tasks_high = tasks.select do |task|
      task.priority == :high
    end
...
module Mailing
  ...
  def receive(mail, sender)
    self.class::mailbox << mail unless reject?(sender)
  end
  ...
  def send(destination, mail)
    "Sending #{mail.text} from #{self.class::name} to: #{destination}"
